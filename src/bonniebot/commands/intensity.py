"""The /intensity slash command — set or view preferred intensity level."""

from __future__ import annotations

from typing import TYPE_CHECKING

import discord
from discord import app_commands
from discord.ext import commands

from common.utils.logger import get_logger

if TYPE_CHECKING:
    from bonniebot.bot import BonnieBot

log = get_logger(__name__)


class IntensityCog(commands.Cog):
    """Slash command for managing intensity preference."""

    def __init__(self, bot: BonnieBot) -> None:
        self.bot = bot

    @app_commands.allowed_installs(guilds=True, users=True)
    @app_commands.allowed_contexts(guilds=True, dms=True, private_channels=True)
    @app_commands.command(name="intensity", description="Set or view your preferred intensity")
    @app_commands.describe(intensity="Intensity level (1-3): 1=mild, 2=medium, 3=maximum")
    async def intensity(
        self,
        interaction: discord.Interaction,
        intensity: app_commands.Range[int, 1, 3] | None = None,
    ) -> None:
        user_id = interaction.user.id

        if intensity is not None:
            await self.bot.intensity_store.set_intensity(user_id, intensity)
            log.info("intensity_set", user_id=user_id, intensity=intensity)
            await interaction.response.send_message(
                f"Got it! Your intensity is now **{intensity}** (1=mild, 2=medium, 3=maximum)"
            )
        else:
            was_explicitly_set = await self.bot.intensity_store.was_set(user_id)
            log.info("intensity_get", user_id=user_id, was_set=was_explicitly_set)

            if was_explicitly_set:
                current = await self.bot.intensity_store.get_intensity(user_id)
                await interaction.response.send_message(
                    f"Your current intensity is **{current}** (1=mild, 2=medium, 3=maximum)"
                )
            else:
                await interaction.response.send_message(
                    "You haven't set a preferred intensity yet. "
                    "Use `/intensity <1-3>` to set one, or the bot will use the "
                    "default (3=maximum)."
                )


async def setup(bot: commands.Bot) -> None:
    await bot.add_cog(IntensityCog(bot))  # type: ignore[arg-type]
