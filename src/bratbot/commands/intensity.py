    ...
    async def intensity(
        ...
        if intensity is not None:
            ...
            await interaction.response.send_message(
                f"✓ Your brat intensity is now **{intensity}** (1=mild, 2=medium, 3=maximum)"
            )
        else:
            ...
            if was_explicitly_set:
                ...
                await interaction.response.send_message(
                    f"Your current brat intensity is **{current}** (1=mild, 2=medium, 3=maximum)"
                )
            else:
                await interaction.response.send_message(
                    "You haven't set a preferred intensity yet. "
                    "Use `/intensity <1-3>` to set one, or the bot will use the default (3=maximum)."
                )
